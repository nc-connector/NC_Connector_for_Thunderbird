...
- Error logging follows one rule: use `NCLogContext.safeConsoleError(...)` for shared helpers and UI pages. Keep a local `log...Error()` wrapper only when it adds runtime-prefix logic, callback shape, or logger-adapter behavior.
- Direct `console.error(...)` stays limited to logging internals, early bootstrap, and background event handlers that already own their `[NCBG]` prefix.
...
- `policyRuntime` reads the optional `license_status`, `access_status`, `can_manage_license`, `license_activation.state`, `license_connection_error`, `license_last_sync_at_iso` and `license_offline_until_iso` fields. `policyState.getStatusNotice` selects a single display reason; `wizardPolicyUi` formats it for settings, Share/Talk warnings and disabled-feature tooltips. VFS status carries the same plain notice descriptor. These fields do not change the existing validity, seat, mode or policy-domain gates, and dates are used only for display.
- Notice precedence separates full administrators from ordinary users without a seat, then distinguishes invalid entitlement, capacity limits and genuinely suspended seats before grace/synchronization notices. Old payloads retain generic access wording. Only `suspended_overlimit` uses suspended-seat wording. Blocking `access_status` takes precedence over commercial `license_status`; a commercial grace period never hides an activation/offline refusal.

...
- The Sharing wizard keeps **+ Other source** reachable without a connection and shows the current setup state. Its actions open the local VFS options tab, open the public Thunderbird Add-ons VFS search in a normal Thunderbird content tab, or refresh discovery. These navigation actions add no permission and do not modify Toolkit code. A populated queue is warned before entering an enablement or permission-restoration flow that can reload the extension.
